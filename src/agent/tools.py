...
def _publish_skill(
    ...
    """Publish a skill document to SKILLS.md in the repo root.

    If SKILLS.md already has a section for the framework (## <name>),
    that section is replaced. Otherwise the new section is appended.
    Also invalidates the repo knowledge cache so the UI picks it up.
    """
    framework = args.get("framework_name", "").strip()
    content = args.get("skill_content", "").strip()
    if not framework:
        return "Error: framework_name is required"
    if not content:
        return "Error: skill_content is required"
    ...
    new_section = f"{header}\n_Extracted: {date_str}_\n\n{content}"

    if skills_path.exists():
        existing = skills_path.read_text(encoding="utf-8")
        # Check if a section for this framework already exists
        # Pattern: ## Framework Name\n ... until next ## or EOF
        pattern = re.compile(
            rf"^## {re.escape(framework)}\s*\n.*?(?=\n## |\Z)",
        ...
        updated = (
            "# Skills\n"
            "<!-- Auto-generated by Skill_Extractor. -->\n\n"
            + new_section + "\n"
        )

    skills_path.write_text(updated, encoding="utf-8")

    # Invalidate repo knowledge cache so UI and future agent sessions pick it up
    try:
        from src.services.cache import repo_cache
        from src.agent.knowledge import compute_repo_id
        repo_id = compute_repo_id(str(repo_root), "")
        repo_cache.invalidate(repo_id, "repo_knowledge")
    except Exception:
        pass

    return (
        f"Published skill '{framework}' to SKILLS.md ({len(updated)} chars total). "
        "The skill is now visible in Repos > [repo] > SKILLS.md and will be "
        "injected into future agent sessions automatically."
    )


def _list_dependencies(repo_root: Path) -> str:
    """Parse build files and return structured dependency info.

    Classifies each dependency as 'known to LLM' (open-source, skip source
    inspection) or 'internal/unknown' (enterprise, needs source download).
    """
    from src.agent.artifact_repo import (
        list_dependencies, is_known_to_llm, is_npm_known_to_llm,
    )

    result = list_dependencies(str(repo_root))
    deps = result.get("dependencies", [])
    hints = result.get("framework_hints", [])
    build_file = result.get("build_file", "")
    build_tool = result.get("build_tool", "")

    if not deps:
        return f"Build tool: {build_tool}\nNo dependencies found (build file: {build_file or 'not found'})"
    ...
    for d in deps:
        if build_tool == "npm":
            known = is_npm_known_to_llm(d.get("artifactId", ""))
        else:
            known = is_known_to_llm(d.get("groupId", ""), d.get("artifactId", ""))
        if known:
            known_deps.append(d)
        else:
    ...
    lines = [
        ...
        f"Build file: {build_file}",
        f"Total dependencies: {len(deps)}",
        f"  Internal/enterprise (NEED source inspection): {len(internal_deps)}",
        f"  Well-known open-source (LLM already knows): {len(known_deps)}",
    ]
    if hints:
        lines.append(f"Detected frameworks: {', '.join(hints)}")
    lines.append("")

    # Internal deps first — these are the priority
    if internal_deps:
        lines.append("=" * 60)
        lines.append("INTERNAL / ENTERPRISE DEPENDENCIES (download sources for these)")
        lines.append("These are NOT in LLM training data — you MUST inspect their source code.")
        lines.append("=" * 60)
        for d in internal_deps:
            lines.append(f"  {d['coordinate']}  [{d.get('scope', 'compile')}]")
        lines.append("")

    # Known deps — just list for reference
    if known_deps:
        lines.append("-" * 60)
        lines.append("WELL-KNOWN DEPENDENCIES (LLM already trained on these)")
        lines.append("Skip source download. Only document repo-specific usage conventions.")
        lines.append("-" * 60)
        for d in known_deps:
            lines.append(f"  {d['coordinate']}  [{d.get('scope', 'compile')}]")
        lines.append("")
...
def _download_dependencies(
    ...
    """Download dependency source code using the configured artifact repository."""
    if not agent_config:
        return "Error: agent config not available"

    ar_config = agent_config.get("artifact_repository", {})
    if not ar_config.get("enabled", False):
        return (
            "Error: Artifact repository is not enabled. "
            "Configure it in Settings > Configuration > Artifact Repository."
        )

    from src.agent.artifact_repo import prepare_dependency_env

    dep_env = prepare_dependency_env(ar_config, str(repo_root))
    if dep_env.get("error"):
        return f"Error: {dep_env['error']}"

    download_cmd = dep_env.get("download_cmd", "")
    if not download_cmd:
        return "Error: Could not determine download command for this project."

    # Adjust command based on args
    include_sources = args.get("include_sources", True)
    scope = args.get("scope", "all")
    build_tool = dep_env["build_tool"]

    if build_tool == "maven":
        if not include_sources:
            download_cmd = download_cmd.replace("-Dclassifier=sources ", "")
        if scope and scope != "all":
            download_cmd += f" -DincludeScope={scope}"
    elif build_tool == "gradle":
        if scope and scope != "all":
            download_cmd += f" --configuration {scope}"

    # Run the command
    result = run_command(repo_root, download_cmd, timeout=300, agent_config=agent_config)

    cache_dir = dep_env.get("source_cache_dir", "")
    return (
        f"Build tool: {build_tool}\n"
...
# --- Dependency download tool schema ---

_PUBLISH_SKILL_SCHEMA = _tool("publish_skill",
    "Publish an extracted skill document to the repo's SKILLS.md file. "
    "This persists the skill so it is visible in the Repos UI and automatically "
    "injected into future agent sessions as repo knowledge. "
    "If SKILLS.md already contains a section for this framework, it is replaced; "
    "otherwise the new section is appended.",
    {"framework_name": {"type": "string", "description": "Framework/library name (e.g. SmartSpec, Spring Boot)"},
     "skill_content": {"type": "string", "description": "The complete skill document in Markdown (Contract, Repo Conventions, Code Generation Guide, Pitfalls sections)"}},
    required=["framework_name", "skill_content"],
)

_LIST_DEPS_SCHEMA = _tool("list_dependencies",
    "Parse the project's build file (pom.xml, build.gradle, package.json) and return "
    "a structured list of all dependencies with groupId, artifactId, version, and scope. "
    "Also auto-detects known frameworks (Spring, JUnit, React, etc.). "
    "Use this BEFORE download_dependencies to know exactly what to inspect.",
    {},
)

_DOWNLOAD_DEPS_SCHEMA = _tool("download_dependencies",
    "Download dependency source code for the current repository using the configured "
    "enterprise artifact repository. Downloads sources into the local cache "
    "(~/.m2/repository for Maven, node_modules for npm) so they can be read.",
    {"include_sources": {"type": "boolean", "description": "Download source JARs (default: true)"},
     "scope": {"type": "string", "description": "Dependency scope filter (e.g. 'compile', 'test', 'all'). Default: all"}},
)

MCP_TOOLS = [_MCP_CONNECT_SCHEMA, _MCP_CALL_SCHEMA]


def _artifact_repo_enabled(agent_config: Optional[dict] = None) -> bool:
    """Check if artifact repository integration is enabled in agent config."""
    if agent_config is None:
        return False
    ar = agent_config.get("artifact_repository", {})
    return bool(ar.get("enabled", False))
...
def build_agent_tools(agent_config: Optional[dict] = None, code_index: Optional["CodeIndex"] = None) -> list[dict]:
    ...
    tools.append(_PUBLISH_SKILL_SCHEMA)
    tools.append(_LIST_DEPS_SCHEMA)
    if _artifact_repo_enabled(agent_config):
        tools.append(_DOWNLOAD_DEPS_SCHEMA)
...
def execute_tool(
    ...
    *code_index*: if provided, used by code_index tools.
    ...
    # --- Skill publishing tool ---
    if tool_name == "publish_skill":
        return _publish_skill(repo, args, agent_config)

    # --- Dependency listing tool ---
    if tool_name == "list_dependencies":
        return _list_dependencies(repo)

    # --- Dependency download tool ---
    if tool_name == "download_dependencies":
        return _download_dependencies(repo, args, agent_config)
